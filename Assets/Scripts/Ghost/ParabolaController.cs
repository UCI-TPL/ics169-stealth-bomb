...
public class ParabolaController : MonoBehaviour
{
    ...
    */
    ...
    public class ParabolaFly
    {
        ...
        int max;  //of course this will change when we have multiple maps, this will be moved!
        int min;

        public ParabolaFly(Transform ParabolaRoot, int max, int min)
        {
            this.max = max;
            this.min = min;
        ...
        public Vector3 GetPositionAtZ(float z) //uses the Z position of the cursor to find a position for the ghost 
        {
            float percent = ((z-min)  / (max-min)  )    ;
            return parabolas[0].GetPositionAtLength(percent * parabolas[0].Length);
        }

        public int GetParabolaIndexAtPosition(float z) //ignore this
        {
            int index = 0; 
            for(int i = min; i <= max; i+=9) //since the increment for now is 9
            {
                if(z <= i)
                    return index;
                index++;
